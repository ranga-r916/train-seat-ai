...
def verify_razorpay_signature(order_id: str, payment_id: str, signature: str) -> bool:
    ...
    if "rzp_test_" in settings.RAZORPAY_KEY_ID or settings.RAZORPAY_KEY_SECRET == "your-razorpay-secret":
...
def confirm_payment(
    ...
    if booking.family_booking_id:
        bookings_to_confirm = db.query(Booking).filter(
            Booking.family_booking_id == booking.family_booking_id
        ).all()
    else:
    ...
    for b in bookings_to_confirm:
        ...
        pay_rec = db.query(Payment).filter(Payment.booking_id == b.id).first()
        if pay_rec:
            ...
            pay_rec.method = PaymentMethod[data.method.upper()]
            ...
            pay_rec = Payment(
                ...
                method=PaymentMethod[data.method.upper()],
